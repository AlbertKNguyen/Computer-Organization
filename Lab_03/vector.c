...
void vector_delete(vector_t *v) {
	...
	/* ADD CODE HERE */
	// free memory allocated for data pointer
	free(v->data);
	// free memory allocated for struct pointer
	free(v);
}
...
void vector_set(vector_t *v, size_t loc, int value) {
	...
	/* ADD CODE HERE */
	//if location value is valid, change value
	if(loc < v->size) {
		v->data[loc] = value;
	}
	//else allocate memory for vector pointer and data pointer and change value to new size and data.
	else {
		for(int counter = v->size; counter < loc; counter++){
            v->data[counter]= 0;
        }
		v->data[loc] = value;
		v->size = loc + 1;
